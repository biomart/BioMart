...
public class ReorderTabDialog extends JDialog implements ActionListener, ListSelectionListener{
	...
	public ReorderTabDialog(GuiContainer gc) {
		this.gc = (GuiContainer)gc.getParent();
		this.init();
		this.setModal(true);
		this.pack();
	...
	private void init() {
		...
		for(GuiContainer mp: this.gc.getGuiContainerList()) {
...
//		buttonPanel.add(saveButton);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getActionCommand().equals(Resources.get("CLOSE"))) {
			...
			Collections.swap(this.gc.getGuiContainerList(),index-1,index);
			this.changed = true;
			...
			model.insertElementAt(obj, index+1);
			this.configList.setSelectedIndex(index+1);
			Collections.swap(this.gc.getGuiContainerList(),index,index+1);
			this.changed = true;
		}	
	}
