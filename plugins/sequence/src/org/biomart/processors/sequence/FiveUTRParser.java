        ...
        if (!chr.equals("")){
            if (strand.equals("-1")){
                ...
                if(getSequence(chr,end.get(terminalExonRank)-codingOffset+2,end.get(terminalExonRank)).equals("")){
                	if(terminalExonRank>0)
                        sequence.append(getReverseComplement(getSequence(chr,start.get(terminalExonRank-1)-downstreamFlank,start.get(terminalExonRank-1)-1)));

                } else {
                    sequence.append(getReverseComplement(getSequence(chr,end.get(terminalExonRank)-codingOffset+2-downstreamFlank,end.get(terminalExonRank))));
                }
                if(sequence.length()!=0)
                    sequence.insert(0, getReverseComplement(getSequence(chr,end.get(0)+1,end.get(0)+upstreamFlank)));
                ...
                if(getSequence(chr,start.get(terminalExonRank),start.get(terminalExonRank)+codingOffset-2).equals("")){
                	if(terminalExonRank > 0)
                		sequence.append(getSequence(chr,end.get(terminalExonRank-1)+1,end.get(terminalExonRank-1)+downstreamFlank));
                } else {
                	sequence.append(getSequence(chr,start.get(terminalExonRank),start.get(terminalExonRank)+codingOffset-2+downstreamFlank));
                }
                if(sequence.length()!=0)
                    sequence.insert(0,getSequence(chr,start.get(0)-upstreamFlank,start.get(0)-1));
            }
        }
