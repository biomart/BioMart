...
!function(e,t){"use strict";function n(e){e=t.select(e);var n=window.Blob||window.WebKitBlob||window.MozBlob,r=e.select("svg").attr("version",1.1).attr("xmlns","http://www.w3.org/2000/svg").node().parentNode.innerHTML,o=-1!=navigator.userAgent.indexOf("Safari")&&-1==navigator.userAgent.indexOf("Chrome");if(o){var a="data:image/svg+xml;utf8,"+r;window.open(a,"download")}else{var i=new n([r],{type:"data:image/svg+xml"});saveAs(i,"enrichment-network.svg")}}function r(e){var t=JSON.stringify(e),n=new Blob([t],{type:"data:text/json;charset=utf-8"});saveAs(n,"enrichment-network.json")}function o(e){for(var t={},n=0,r=e.edges.length;r>n;++n){var o=e.edges[n];(t[o.target.id]||(t[o.target.id]=[])).push(o.source.id)}var a="",i="	";for(var s in t){var c=e.nodes.filter(function(e){return e.id===s}),l=c[0];a+=[s,l.description,t[s].join(","),l.pvalue].join(i)+"\n"}var d=new Blob([a],{type:"data:text/plain;charset=utf-8"});saveAs(d,"enrichment-network.tsv")}function a(e){e=t.select(e);var n=(t.select("body").append("canvas").attr("id","canvas").style("display","none"),e.select("svg").node().parentNode.innerHTML);canvg("canvas",n);var r=document.getElementById("canvas"),o=-1!=navigator.userAgent.indexOf("Safari")&&-1==navigator.userAgent.indexOf("Chrome");if(o){var a=r.toDataURL("image/png;base64");window.open(a,"download")}else r.toBlob(function(e){saveAs(e,"enrichment-network.png")},"image/png");t.select("#canvas").remove()}var i=function(e){return function(t,n,r){function o(e){return k(e.id)}function a(e){return"term"===e.type}function i(e){return parseFloat(e.pvalue)}function s(){D.attr("transform","translate("+e.event.translate+")scale("+e.event.scale+")")}function c(){e.event.sourceEvent.stopPropagation(),e.select(this).classed("dragging",!0)}function l(){e.select(this).classed("dragging",!1)}function d(e,t){var n=g(e,t);return n}function u(e,t){for(var n=0,r=t.length;r>n;++n)f(e,t[n])}function f(e,t){for(var n,r,o=t.source,a=t.target,i=0,s=e.length;s>i&&("string"==typeof o||"string"==typeof a);)n=e[i],r=n.id,o===r?t.source=o=n:a===r&&(t.target=a=n),++i}function g(e,t){for(var n,r,o,a=[],i=!1,s=0,c=0,l=e.length;l>c;++c){n=e[c];for(var d=0+s,u=t.length;u>d;++d)r=t[d],r.target===n&&-1===a.indexOf(r.target)?(a.push(r.source),i=!0):r.source===n&&-1===a.indexOf(r.source)&&(a.push(r.target),i=!0),i&&(i=!1,o=t[s],t[s]=r,t[d]=o,++s)}return[a,s>0?t.slice(0,s):[]]}function v(e){var t=M[1]-M[0];return e.r=e.pr=0===t?13:30*(1-(parseFloat(e.pvalue)-M[0])/(M[1]-M[0]))+13}function p(e,t){var n=e.length,r=360/n*Math.PI/180,o=r,a=t.centre||[0,0],i=t.radius||50,s=t.offset||20;e.forEach(function(e){e.x=a[0]+i*Math.cos(o)+(-s+2*Math.random()*s),e.y=a[1]+i*Math.sin(o)+(-s+2*Math.random()*s),o+=r})}function h(e){return e.x}function m(e){return e.y}function w(){I.attr({cx:h,cy:m}),T.attr({x1:function(e){return e.source.x},y1:function(e){return e.source.y},x2:function(e){return e.target.x},y2:function(e){return e.target.y}}),R.attr("transform",function(e){return"translate("+e.x+","+e.y+")"})}function y(){H.nodes().forEach(function(e){e.fixed=!0})}function x(t){if(t.pr===t.r){var n=e.select(this);n.transition().attr("r",t.r=1.5*t.r),n.classed("node-selected",!0)}}function b(t){var n=e.select(this);e.select(this).transition().attr("r",t.r=t.pr),n.classed("node-selected",!1)}function k(e){return"e"+e.replace(/[:;,\.]*/g,"")}r=r||{};var A=e.select(r.container),E=r.showNth||5,O=r.margin||{top:20,right:30,bottom:20,left:30},j=(r.width||1200)-O.right-O.left,N=(r.height||800)-O.top-O.bottom;u(t,n);var _=t.filter(a).splice(0,E),M=e.extent(_,i),$=d(_,n),B=$[0];n=$[1];var S=Object.keys(B[0]),z=Object.keys(_[0]).filter(function(e){return"$"!==e[0]&&"_"!==e[0]});$=null;var C=e.behavior.zoom().scaleExtent([0,4]).on("zoom",s),P=A.append("svg:svg").attr({title:"test2",version:1.1,xmlns:"http://www.w3.org/2000/svg",width:j+O.right+O.left,height:N+O.top+O.bottom}).style("cursor","pointer").append("svg:g").attr("transform",["translate(",O.left,",",O.top,")"].join("")).call(C).on("mousedown.zoom",null),q=(P.append("rect").attr("width",j).attr("height",N).style("fill","none").style("pointer-events","all"),e.tip().attr("class","d3-tip").offset([-10,0]).html(function(e){var t="gene"===e.type?S:z;return t.reduce(function(t,n){return t+'<strong class="d3-tip-prop">'+n+"</strong>: "+e[n]+"<br/>"},"")})),D=P.append("svg:g").attr({"class":"enrichment-vis",transform:["translate(",O.left,",",O.top,")"].join("")}),F=3.5*v({pvalue:M[0]});p(_,{centre:[j/2,N/2],radius:5,offset:5}),p(B,{centre:[j/2,N/2],radius:F,offset:5});var H=e.layout.force().size([j,N]).linkDistance(F).nodes(_.concat(B)).links(n).gravity(.4).charge(function(e){return"gene"===e.type?-400:-100*e.weight}).on("tick",w).on("end",y).start().alpha(.02),L=H.drag().on("dragstart",c).on("dragend",l);t=H.nodes();var T=D.selectAll(".line").data(n).enter().append("line").attr("class","line").style({stroke:"#556270","shape-rendering":"geometricPrecision"}).attr("id",function(e,t){return e.index=t,"link"+t});D.selectAll(".term").data(_).enter().append("circle").attr("r",v).attr("id",o).classed({term:!0,node:!0}).style({opacity:.9,fill:"#4ECDC4",stroke:"#4194cd","stroke-width":2}).call(L),D.selectAll(".gene").data(B).enter().append("circle").attr("r",function(e){return e.r=e.pr=10}).attr("id",o).classed({gene:!0,node:!0}).style({opacity:.9,fill:"#C7F464",stroke:"#67c822","stroke-width":2}).call(L).call(q);var I=D.selectAll(".node");I.on("mouseover.node.tip",q.show).on("mouseout.node.tip",q.hide).on("mouseover.node.transition",x).on("mouseout.node.transition",b);var K=D.append("svg:g").attr("class","node-text");K.selectAll(".term-label").data(_).enter().append("svg:g").append("svg:text").attr("class","term-label").style({"text-anchor":"middle","shape-rendering":"crispEdges"}).text(function(e){return e.description.length>28?e.description.substr(0,25)+"...":e.description}),K.selectAll(".gene-label").data(B).enter().append("svg:g").append("svg:text").attr("class","gene-label").style({"text-anchor":"middle","shape-rendering":"crispEdges","font-size":"0.7em"}).text(function(e){return e.id});var R=K.selectAll("g");return{hn:[],remove:function(){D.remove(),A.select("svg").remove()},highlightNode:function(e){var t=document.querySelector("#"+k(e));t&&(x.call(t,t.__data__),this.hn.push(t))},restoreNodes:function(){this.hn.forEach(function(e){b.call(e,e.__data__)}),this.hn=[]}}}}.call(this,t);e.module("martVisualEnrichment.directives").directive("mvGraph",["$rootScope","progressState","$timeout",function(t,s,c){function l(e,l){var d=null;c(function(){var s=l.find("div").eq(0);d=i(e.nodes,e.edges,{container:s[0],width:l.width(),height:l.height()}),l.find(".save-png").on("click",function(){a(l.find("div").eq(0)[0])}),l.find(".save-svg").on("click",function(){n(l.find("div").eq(0)[0])}),l.find(".save-json").on("click",function(){r({nodes:e.nodes,edges:e.edges})}),l.find(".save-tsv").on("click",function(){o({nodes:e.nodes,edges:e.edges})});var c=t.$on("term.mouseover",function(e,t){d.highlightNode(t.id)}),u=t.$on("term.mouseout",function(e,t){d.restoreNodes(t.id)});e.$on("$destroy",function(){d.remove(),c(),u()})},200),s.setState(s.states.NETWORK);var u=!0;e.$watch(function(e){return e.filterPattern},function(e,t){u&&(u=!1)})}return{restrict:"E",replace:!0,templateUrl:"mart-visual-enrichment/app/partials/vis.html",scope:{nodes:"=nodes",edges:"=edges",filterPattern:"="},compile:function(t){var n=e.element(window).prop("innerHeight");return t.css("height",n+"px"),l}}}])}(angular,d3);
