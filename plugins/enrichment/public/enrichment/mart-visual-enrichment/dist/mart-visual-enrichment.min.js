...
!function(e,t){"use strict";function n(e){e=t.select(e);var n=window.Blob||window.WebKitBlob||window.MozBlob,r=e.select("svg").attr("version",1.1).attr("xmlns","http://www.w3.org/2000/svg").node().parentNode.innerHTML;if(o){var i="data:image/svg+xml;utf8,"+r;window.open(i,"_blank")}else{var a=new n([r],{type:"data:image/svg+xml"});saveAs(a,"enrichment-network.svg")}}function r(e){var t=JSON.stringify(e);if(o){var n="data:application/json;utf8,"+t;window.open(n,"_blank")}else{var r=new Blob([t],{type:"data:application/json;charset=utf-8"});saveAs(r,"enrichment-network.json")}}function i(e){for(var t={},n=0,r=e.edges.length;r>n;++n){var i=e.edges[n];(t[i.target.id]||(t[i.target.id]=[])).push(i.source.id)}var a="",s="	";for(var c in t){var l=e.nodes.filter(function(e){return e.id===c}),u=l[0];a+=[c,u.description,t[c].join(","),u.pvalue].join(s)+"\n"}if(o){var d="data:application/json;utf8,"+a;window.open(d,"_blank")}else{var f=new Blob([a],{type:"data:text/plain;charset=utf-8"});saveAs(f,"enrichment-network.tsv")}}function a(e){e=t.select(e);var n=(t.select("body").append("canvas").attr("id","canvas").style("display","none"),e.select("svg").node().parentNode.innerHTML);canvg("canvas",n);var r=document.getElementById("canvas"),i=-1!=navigator.userAgent.indexOf("Safari")&&-1==navigator.userAgent.indexOf("Chrome");if(i){var a=r.toDataURL("image/png;base64");window.open(a,"download")}else r.toBlob(function(e){saveAs(e,"enrichment-network.png")},"image/png");t.select("#canvas").remove()}var o=-1!=navigator.userAgent.indexOf("Safari")&&-1==navigator.userAgent.indexOf("Chrome"),s=function(e){return function(t,n,r){function i(e,t,n){var r=n.e+e*n.a,i=n.f+t*n.d;return{x:r,y:i}}function a(e){return A(e.id)}function o(e){e.selected=!1}function s(e){e.selected=!0}function c(e){return"term"===e.type}function l(e){return parseFloat(e.pvalue)}function u(){K.attr("transform","translate("+e.event.translate+")scale("+e.event.scale+")")}function d(){function t(t){t.px=t.x,t.py=t.y,t.x+=e.event.dx,t.y+=e.event.dy}function n(e){if(e.selected){var n=G.filter(function(e){return e.selected});n.each(t)}else t(e);x()}var r=e.behavior.drag().origin(function(e){return e}).on("drag.graph",n);this.on(".drag",null),this.call(r)}function f(e,t){return p(e,t)}function g(e,t){for(var n=0,r=t.length;r>n;++n)h(e,t[n])}function h(e,t){for(var n,r,i=t.source,a=t.target,o=0,s=e.length;s>o&&("string"==typeof i||"string"==typeof a);)n=e[o],r=n.id,i===r?t.source=i=n:a===r&&(t.target=a=n),++o}function p(e,t){for(var n,r,i,a=[],o=!1,s=0,c=0,l=e.length;l>c;++c){n=e[c];for(var u=0+s,d=t.length;d>u;++u)r=t[u],r.target===n?(o=!0,-1===a.indexOf(r.source)&&a.push(r.source)):r.source===n&&(o=!0,-1===a.indexOf(r.target)&&a.push(r.target)),o&&(o=!1,i=t[s],t[s]=r,t[u]=i,++s)}return[a,s>0?t.slice(0,s):[]]}function v(e){var t=T[1]-T[0];return e.r=e.pr=0===t?20:20*(1-(parseFloat(e.pvalue)-T[0])/(T[1]-T[0]))+20}function m(e,t){var n=e.length,r=360/n*Math.PI/180,i=r,a=t.centre||[0,0],o=t.radius||50,s=t.offset||20;e.forEach(function(e){e.x=a[0]+o*Math.cos(i)+(-s+2*Math.random()*s),e.y=a[1]+o*Math.sin(i)+(-s+2*Math.random()*s),i+=r})}function y(e){var t=10,n=e.r+t,r=e.x-n,i=e.x+n,a=e.y-n,o=e.y+n;return function(n,s,c,l,u){if(n.point&&n.point!==e){var d=e.x-n.point.x,f=e.y-n.point.y,g=Math.sqrt(d*d+f*f),h=e.r+n.point.r+t;h>g&&(g=(g-h)/g*.5,e.x-=d*=g,e.y-=f*=g,n.point.x+=d,n.point.y+=f)}return s>i||r>l||c>o||a>u}}function x(t){G.each(function(n){e.select(this).attr({cx:function(){return t&&(n.x="term"===n.type?n.x+1.71*(z/2-n.x)*t.alpha:Math.max(n.r,Math.min(z-n.r,n.x))),n.x},cy:function(){return t&&(n.y="term"===n.type?n.y+1.71*(B/2-n.y)*t.alpha:Math.max(n.r,Math.min(B-n.r,n.y))),n.y}})}),U.attr({x1:function(e){return e.source.x},y1:function(e){return e.source.y},x2:function(e){return e.target.x},y2:function(e){return e.target.y}}),Z.attr("transform",function(e){return"translate("+e.x+","+e.y+")"})}function w(n){for(var r=e.geom.quadtree(t),i=0,a=t.length;++i<a;)r.visit(y(t[i]));+new Date-J>V&&Q.stop(),x(n)}function b(t){if(t.pr===t.r){var n=e.select(this);n.transition().attr("r",t.r=1.5*t.r),n.classed("node-selected",!0)}}function k(t){var n=e.select(this);e.select(this).transition().attr("r",t.r=t.pr),n.classed("node-selected",!1)}function A(e){return"e"+e.replace(/[:;,\.]*/g,"")}function M(e){var r,i,a,o=S||(S=[]),s=$||($=[]);if(!o.length){for(r=t.length,i=n.length,o=new Array(r),a=0;r>a;++a)o[a]=[],s[a]=[];for(a=0;i>a;++a){var c=n[a];o[c.source.index].push(c.target),o[c.target.index].push(c.source),s[c.source.index].push(c),s[c.target.index].push(c)}}return o[e]}function E(e){var t=M(e),n=$;return t.concat(n[e])}function _(){t.forEach(o),n.forEach(o),I.hide()}r=r||{};var j=e.select(r.container),N=r.showNth||5,O=r.margin||{top:20,right:30,bottom:20,left:30},z=(r.width||1200)-O.right-O.left,B=(r.height||800)-O.top-O.bottom,S=[],$=[];g(t,n);var D=t.filter(c).splice(0,N),T=e.extent(D,l),q=f(D,n),F=q[0];n=q[1];var P=Object.keys(F[0]),C=Object.keys(D[0]);q=null;var H=e.behavior.zoom().scaleExtent([0,4]).on("zoom",u),L=j.append("svg:svg").attr({title:"test2",version:1.1,xmlns:"http://www.w3.org/2000/svg",width:z+O.right+O.left,height:B+O.top+O.bottom}).style("cursor","pointer");L.append("rect").attr("width","100%").attr("height","100%").style("fill","#fff"),L=L.append("svg:g").attr("transform",["translate(",O.left,",",O.top,")"].join("")).call(H).on("mousedown.zoom",null).on("dblclick.zoom",null);var I=(L.append("rect").attr("width",z).attr("height",B).style("fill","white").style("pointer-events","all"),e.tip().attr("class","d3-tip").offset([-20,0]).html(function(e){var t="gene"===e.type?P:C;return t.reduce(function(t,n){return t+'<strong class="d3-tip-prop">'+n+"</strong>: "+e[n]+"<br/>"},"")})),K=L.append("svg:g").attr("class","enrichment-vis").style("visibility","hidden"),R=v({pvalue:T[0]});m(D,{centre:[z/2,B/2],radius:5,offset:5}),m(F,{centre:[z/2,B/2],radius:5*R,offset:5}),t=D.concat(F);var U=K.selectAll(".line").data(n).enter().append("line").attr("class","line").style({stroke:"#F28882","shape-rendering":"geometricPrecision"}).attr("id",function(e,t){return e.index=t,"link"+t}),W=e.scale.category20();K.selectAll(".term").data(D).enter().append("circle").attr("r",v).attr("id",a).classed({term:!0,node:!0}).style({opacity:.9,fill:function(e){return W(e.pvalue)},stroke:"#333","stroke-width":2}),K.selectAll(".gene").data(F).enter().append("circle").attr("r",function(e){return e.r=e.pr=11}).attr("id",a).classed({gene:!0,node:!0}).style({opacity:.9,fill:"#D7D2AE",stroke:"#F28882","stroke-width":2}).call(I);var G=K.selectAll(".node");G.call(d).on("mouseover.node.tip",I.show).on("mouseout.node.tip",I.hide).on("mouseover.node.transition",b).on("mouseout.node.transition",k).on("dblclick.hightlightneighbors",function(t,n){if(_(),"circle"===e.event.target.tagName){var r=E(n).concat([t]);r.forEach(s),G.classed("selected",function(e){return e.selected}),U.classed("selected",function(e){return e.selected})}});{var J=+new Date,V=5e3,Q=e.layout.force().size([z,B]).linkDistance(3*R).nodes(t).links(n).gravity(.5).charge(function(e){return"gene"===e.type?-400:-150*e.weight}).on("tick",w).on("start",function(){G.transition().duration(2e3).attrTween("r",function(t){var n=e.interpolate(1,t.r);return function(e){return t.r=n(e)}})}).start(),X=e.svg.brush().x(e.scale.identity().domain([0,z])).y(e.scale.identity().domain([0,B])).on("brushend",function(){e.event.target.clear(),e.select(this).call(e.event.target)}).on("brush.clear",function(){U.classed("selected",o)}).on("brush",function(){function t(e){var t=i(e.x,e.y,this.getCTM());return e.selected=n[0][0]<=t.x&&t.x<n[1][0]&&n[0][1]<=t.y&&t.y<n[1][1]}var n=e.event.target.extent();G.classed("selected",t)});L.insert("g",".enrichment-vis").attr("class","brush").call(X)}L.select(".brush .background").style("fill","white");var Y=K.append("svg:g").attr("class","node-text");Y.selectAll(".term-label").data(D).enter().append("svg:g").append("svg:text").attr("class","term-label").style({"text-anchor":"middle","shape-rendering":"crispEdges"}).text(function(e){return e.description.length>28?e.description.substr(0,25)+"...":e.description}),Y.selectAll(".gene-label").data(F).enter().append("svg:g").append("svg:text").attr("class","gene-label").style({"text-anchor":"middle","shape-rendering":"crispEdges","font-size":"0.7em"}).text(function(e){return e.id});var Z=Y.selectAll("g");return K.style("visibility",null),{hn:[],remove:function(){K.remove(),j.select("svg").remove()},highlightNode:function(e){var t=document.querySelector("#"+A(e));t&&(b.call(t,t.__data__),this.hn.push(t))},restoreNodes:function(){this.hn.forEach(function(e){k.call(e,e.__data__)}),this.hn=[]}}}}.call(this,t);e.module("martVisualEnrichment.directives").directive("mvGraph",["$rootScope","progressState","$timeout",function(t,o,c){function l(e,l){function u(){return l.find("div").eq(0)[0]}function d(){return{container:u(),width:l.width(),height:l.height()}}function f(e,t){return e.filter(function(e){return"gene"===e.type||e.description&&e.description.search(t)>-1})}var g=null;c(function(){g=s(e.nodes,e.edges,d()),l.find(".save-png").on("click",function(){a(u())}),l.find(".save-svg").on("click",function(){n(u())}),l.find(".save-json").on("click",function(){r({nodes:e.nodes,edges:e.edges})}),l.find(".save-tsv").on("click",function(){i({nodes:e.nodes,edges:e.edges})});var o=t.$on("term.mouseover",function(e,t){g.highlightNode(t.id)}),c=t.$on("term.mouseout",function(e,t){g.restoreNodes(t.id)});e.$on("$destroy",function(){g.remove(),o(),c()})},200),o.setState(o.states.NETWORK);var h=!0;e.$watch(function(e){return e.filterPattern},function(t,n){h?h=!1:t!==n&&(g.remove(),g=s(f(e.nodes,t),e.edges,d()))})}return{restrict:"E",replace:!0,templateUrl:"mart-visual-enrichment/app/partials/vis.html",scope:{nodes:"=nodes",edges:"=edges",filterPattern:"="},compile:function(t){var n=e.element(window).prop("innerHeight");return t.css("height",n+"px"),l}}}])}(angular,d3);
