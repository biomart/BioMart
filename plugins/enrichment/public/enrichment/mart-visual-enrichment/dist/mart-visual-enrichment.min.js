...
!function(e,t){"use strict";function n(e){e=t.select(e);var n=e.select("svg").node().parentNode.innerHTML;if(o){var r="data:image/svg+xml;base64,"+btoa(n);window.open(r,"_blank")}else{var a=new Blob([n],{type:"data:image/svg+xml;charset=base64"});saveAs(a,"enrichment-network.svg")}}function r(e){var t=JSON.stringify(e);if(o){var n="data:application/json;charset=utf-8,"+t;window.open(n,"_blank")}else{var r=new Blob([t],{type:"data:application/json;charset=utf-8"});saveAs(r,"enrichment-network.json")}}function a(e){for(var t={},n=0,r=e.edges.length;r>n;++n){var a=e.edges[n];(t[a.target.id]||(t[a.target.id]=[])).push(a.source.id)}var i="",s="	";for(var c in t){var l=e.nodes.filter(function(e){return e.id===c}),d=l[0];i+=[c,d.description,t[c].join(","),d.pvalue].join(s)+"\n"}if(o){var u="data:text/plain;charset=utf-8,"+i;window.open(u,"_blank")}else{var f=new Blob([i],{type:"data:text/plain;charset=utf-8"});saveAs(f,"enrichment-network.tsv")}}function i(e){e=t.select(e);var n=(t.select("body").append("canvas").attr("id","canvas").style("display","none"),e.select("svg").node().parentNode.innerHTML);canvg("canvas",n);var r=document.getElementById("canvas");if(o){var a=r.toDataURL("image/jpeg",1);window.open(a,"download")}else r.toBlob(function(e){saveAs(e,"enrichment-network.jpeg")},"image/jpeg",1);t.select("#canvas").remove()}var o=-1!=navigator.userAgent.indexOf("Safari")&&-1==navigator.userAgent.indexOf("Chrome"),s=function(e){return function(t,n,r){function a(e){return w(e.id)}function i(e){return"term"===e.type}function o(e){return parseFloat(e.pvalue)}function s(){D.attr("transform","translate("+e.event.translate+")scale("+e.event.scale+")")}function c(){function t(t){t.px=t.x,t.py=t.y,t.x+=e.event.dx,t.y+=e.event.dy}function n(e){if(e.selected){var n=T.filter(function(e){return e.selected});n.each(t)}else t(e);g()}var r=e.behavior.drag().origin(function(e){return e}).on("drag.graph",n);this.on(".drag",null),this.call(r)}function l(e,t){return f(e,t)}function d(e,t){for(var n=0,r=t.length;r>n;++n)u(e,t[n])}function u(e,t){for(var n,r,a=t.source,i=t.target,o=0,s=e.length;s>o&&("string"==typeof a||"string"==typeof i);)n=e[o],r=n.id,a===r?t.source=a=n:i===r&&(t.target=i=n),++o}function f(e,t){for(var n,r,a,i=[],o=!1,s=0,c=0,l=e.length;l>c;++c){n=e[c];for(var d=0+s,u=t.length;u>d;++d)r=t[d],r.target===n?(o=!0,-1===i.indexOf(r.source)&&i.push(r.source)):r.source===n&&(o=!0,-1===i.indexOf(r.target)&&i.push(r.target)),o&&(o=!1,a=t[s],t[s]=r,t[d]=a,++s)}return[i,s>0?t.slice(0,s):[]]}function p(e){var t=E[1]-E[0];return e.r=e.pr=0===t?13:13*(1-(parseFloat(e.pvalue)-E[0])/(E[1]-E[0]))+10}function v(e,t){var n=e.length,r=360/n*Math.PI/180,a=r,i=t.centre||[0,0],o=t.radius||50,s=t.offset||20;e.forEach(function(e){e.x=i[0]+o*Math.cos(a)+(-s+2*Math.random()*s),e.y=i[1]+o*Math.sin(a)+(-s+2*Math.random()*s),a+=r})}function h(e){var t=10,n=e.r+t,r=e.x-n,a=e.x+n,i=e.y-n,o=e.y+n;return function(n,s,c,l,d){if(n.point&&n.point!==e){var u=e.x-n.point.x,f=e.y-n.point.y,p=Math.sqrt(u*u+f*f),v=e.r+n.point.r+t;v>p&&(p=(p-v)/p*.5,e.x-=u*=p,e.y-=f*=p,n.point.x+=u,n.point.y+=f)}return s>a||r>l||c>o||i>d}}function g(t){T.each(function(n){e.select(this).attr({cx:function(){return t&&(n.x="term"===n.type?n.x+1.71*(j/2-n.x)*t.alpha:Math.max(n.r,Math.min(j-n.r,n.x))),n.x},cy:function(){return t&&(n.y="term"===n.type?n.y+1.71*(M/2-n.y)*t.alpha:Math.max(n.r,Math.min(M-n.r,n.y))),n.y}})}),H.attr({x1:function(e){return e.source.x},y1:function(e){return e.source.y},x2:function(e){return e.target.x},y2:function(e){return e.target.y}}),U.attr("transform",function(e){return"translate("+e.x+","+e.y+")"})}function m(n){for(var r=e.geom.quadtree(t),a=0,i=t.length;++a<i;)r.visit(h(t[a]));n.alpha<.0075&&!C&&(D.style("visibility",null),C=!0),g(n)}function y(t){if(t.pr===t.r){var n=e.select(this);n.transition().attr("r",t.r=1.5*t.r),n.classed("node-selected",!0)}}function x(t){var n=e.select(this);e.select(this).transition().attr("r",t.r=t.pr),n.classed("node-selected",!1)}function w(e){return"e"+e.replace(/[:;,\.]*/g,"")}r=r||{};var b=e.select(r.container),k=r.showNth||5,A=r.margin||{top:20,right:30,bottom:20,left:30},j=(r.width||1200)-A.right-A.left,M=(r.height||800)-A.top-A.bottom;d(t,n);var _=t.filter(i).splice(0,k),E=e.extent(_,o),N=l(_,n),O=N[0];n=N[1];var $=Object.keys(O[0]),q=Object.keys(_[0]).filter(function(e){return"$"!==e[0]&&"_"!==e[0]});N=null;var S=e.behavior.zoom().scaleExtent([0,4]).on("zoom",s),z=b.append("svg:svg").attr({title:"test2",version:1.1,xmlns:"http://www.w3.org/2000/svg",width:j+A.right+A.left,height:M+A.top+A.bottom}).style("cursor","pointer");z.append("rect").attr("width","100%").attr("height","100%").style("fill","#fff"),z=z.append("svg:g").attr("transform",["translate(",A.left,",",A.top,")"].join("")).call(S).on("mousedown.zoom",null);var B=(z.append("rect").attr("width",j).attr("height",M).style("fill","white").style("pointer-events","all"),e.tip().attr("class","d3-tip").offset([-10,0]).html(function(e){var t="gene"===e.type?$:q;return t.reduce(function(t,n){return t+'<strong class="d3-tip-prop">'+n+"</strong>: "+e[n]+"<br/>"},"")})),D=z.append("svg:g").attr("class","enrichment-vis").style("visibility","hidden"),P=p({pvalue:E[0]});v(_,{centre:[j/2,M/2],radius:5,offset:5}),v(O,{centre:[j/2,M/2],radius:5*P,offset:5});var F=e.layout.force().size([j,M]).linkDistance(3*P).nodes(_.concat(O)).links(n).gravity(.5).charge(function(e){return"gene"===e.type?-400:-150*e.weight}).on("tick",m).start().alpha(.0087);t=F.nodes();var H=D.selectAll(".line").data(n).enter().append("line").attr("class","line").style({stroke:"#556270","shape-rendering":"geometricPrecision"}).attr("id",function(e,t){return e.index=t,"link"+t}),L=e.scale.category20();D.selectAll(".term").data(_).enter().append("circle").attr("r",p).attr("id",a).classed({term:!0,node:!0}).style({opacity:.9,fill:function(e){return L(e.pvalue)},stroke:"#333","stroke-width":2}),D.selectAll(".gene").data(O).enter().append("circle").attr("r",function(e){return e.r=e.pr=8}).attr("id",a).classed({gene:!0,node:!0}).style({opacity:.9,fill:"#D7D2AE",stroke:"#F28882","stroke-width":2}).call(B);var T=D.selectAll(".node");T.call(c).on("mouseover.node.tip",B.show).on("mouseout.node.tip",B.hide).on("mouseover.node.transition",y).on("mouseout.node.transition",x);{var I=e.svg.brush().x(e.scale.identity().domain([0,j])).y(e.scale.identity().domain([0,M])).on("brushend",function(){e.event.target.clear(),e.select(this).call(e.event.target)}).on("brush",function(){function t(e){return e.selected=n[0][0]<=e.x&&e.x<n[1][0]&&n[0][1]<=e.y&&e.y<n[1][1]}var n=e.event.target.extent();T.classed("selected",t)});z.insert("g",".enrichment-vis").attr("class","brush").call(I)}z.select(".brush .background").style("fill","white");var R=D.append("svg:g").attr("class","node-text");R.selectAll(".term-label").data(_).enter().append("svg:g").append("svg:text").attr("class","term-label").style({"text-anchor":"middle","shape-rendering":"crispEdges"}).text(function(e){return e.description.length>28?e.description.substr(0,25)+"...":e.description}),R.selectAll(".gene-label").data(O).enter().append("svg:g").append("svg:text").attr("class","gene-label").style({"text-anchor":"middle","shape-rendering":"crispEdges","font-size":"0.7em"}).text(function(e){return e.id});var U=R.selectAll("g"),C=!1;return{hn:[],remove:function(){D.remove(),b.select("svg").remove()},highlightNode:function(e){var t=document.querySelector("#"+w(e));t&&(y.call(t,t.__data__),this.hn.push(t))},restoreNodes:function(){this.hn.forEach(function(e){x.call(e,e.__data__)}),this.hn=[]}}}}.call(this,t);e.module("martVisualEnrichment.directives").directive("mvGraph",["$rootScope","progressState","$timeout",function(t,o,c){function l(e,l){var d=null;c(function(){var o=l.find("div").eq(0);d=s(e.nodes,e.edges,{container:o[0],width:l.width(),height:l.height()}),l.find(".save-jpeg").on("click",function(){i(l.find("div").eq(0)[0])}),l.find(".save-svg").on("click",function(){n(l.find("div").eq(0)[0])}),l.find(".save-json").on("click",function(){r({nodes:e.nodes,edges:e.edges})}),l.find(".save-tsv").on("click",function(){a({nodes:e.nodes,edges:e.edges})});var c=t.$on("term.mouseover",function(e,t){d.highlightNode(t.id)}),u=t.$on("term.mouseout",function(e,t){d.restoreNodes(t.id)});e.$on("$destroy",function(){d.remove(),c(),u()})},200),o.setState(o.states.NETWORK);var u=!0;e.$watch(function(e){return e.filterPattern},function(e,t){u&&(u=!1)})}return{restrict:"E",replace:!0,templateUrl:"mart-visual-enrichment/app/partials/vis.html",scope:{nodes:"=nodes",edges:"=edges",filterPattern:"="},compile:function(t){var n=e.element(window).prop("innerHeight");return t.css("height",n+"px"),l}}}])}(angular,d3);
